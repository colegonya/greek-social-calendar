...
export default async function RootLayout({
  ...
  // Buttons, the today marker, and other brand-primary fills need a text
  // color that stays legible whether a chapter's primary lands dark or
  // light — computed here (not hardcoded white) since the actual hex, custom
  // or default, is only known at render time.
  const primaryTextHex = contrastTextColor(colors.primary || DEFAULT_BRAND_PRIMARY_HEX);
  ...
  return (
    ...
    >
      <body className="flex h-dvh flex-col overflow-hidden" suppressHydrationWarning>
        <style>{`:root{--color-brand-primary-ink:${primaryTextHex};${brandOverrides.map(([name, value]) => `${name}:${value};`).join("")}}`}</style>
        <header className="shrink-0 border-b border-paper-line bg-background">
